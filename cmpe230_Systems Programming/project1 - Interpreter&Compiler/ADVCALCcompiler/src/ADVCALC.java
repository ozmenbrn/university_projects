...
public class ADVCALC {
   ...
   public static <E> void main(String[] args) throws IOException {  
	   ...
	   Scanner input = new Scanner(new File("ADVCALC.txt"));
	   PrintStream out = new PrintStream(new File("ADVCALC.asm"));
	   String str= "";
	   
	   int line = 0;			// how many lines that txt file has
	   ...
	   /**
	    * first read the input from the text file and counts how many lines in there
	    */
	   while(input.hasNext()){
		   line++;
		   input.nextLine();
	   }
	   input = new Scanner(new File("ADVCALC.txt"));
	   ...
	   int[] variable = new int[vari];    //By knowing how many variable THE array can be decleared
   ...
   private static int result(String str, ArrayList<String> list, int[] variable){
	   ...
	   for(int i=0; i<str.length() ; i++){
		   if(str.charAt(i) == 9 || str.charAt(i) == 32 || str.charAt(i) == 61){
		//	   System.out.println(str);
		   }
